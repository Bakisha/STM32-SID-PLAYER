...
void drawShadow(int ii, int c)
{
  ...
  if (line[ii] == bgCol) line[ii] = bgColS; else if (line[ii] == lineCol) line[ii] = lineColS;
  if (c < 0) return;
  if (line[ii] == ballCol1) line[ii] = ballCol1S; else
    for (int i = 0; i < c; i++) if (line[ii] == cols[i]) {
        line[ii] = colsS[i];
        break;
      }
}
...
void MySetup()
{
  ...
  for (int i = 0; i < SCR_HT; i++) {
    drawBg(i);
    lcd.drawImage(0, i, SCR_WD, 1, line);
  }
  //delay(1000);
...
void render()
{
  ...
  for (int b = 0; b < numBalls; b++) {
    ...
    if (ball[b].x < 0) {
      ball[b].x = 0;
      ball[b].xd = -ball[b].xd;
      ball[b].animd = -ball[b].animd;
    }
    if (ball[b].x >= BALL_SWD - ball[b].wd) {
      ball[b].x = BALL_SWD - ball[b].wd;
      ball[b].xd = -ball[b].xd;
      ball[b].animd = -ball[b].animd;
    }
    if (ball[b].y < 0) {
    ...
    if (ball[b].y >= BALL_SHT - ball[b].ht) {
      ball[b].y = BALL_SHT - ball[b].ht;
      ball[b].yd = -ball[b].yd;
    }
  }
  renderFrame();
