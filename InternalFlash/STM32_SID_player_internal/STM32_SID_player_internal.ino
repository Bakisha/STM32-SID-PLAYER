// choose your STM32 microcontroller from board menu (USART port and USB serial can be disabled to increase RAM available)
...
#define FILTER_SENSITIVITY  4// Lower is better. Best is 1 . It will consume irq time (<multiplier> must be higher), but FILTER_FREQUENCY can be set much higher. For value 8, maximum FILTER_FREQUENCY is 8000. For value 16, it's 4000. For 4, it's 12500
...
#define RAM_SIZE 0x4000 + 0x400  // Set the amount of RAM that can be available to emulator (leave at least 2000 bytes for locals) (depends of microcontroller's RAM)
...
const uint8_t period = 1;// period for timer1, for frequency and resolution of PWM in uS
...
const uint8_t multiplier = 32 ; // (byte) set this for interrupt speed in uS (in general, how much slower then real SID). Don't set too small, irq might overlap (music will play  1/2 slower)
...
void setup() {
  ...
  InitHardware(); // Setup timers and interrupts (located in hardware_.ino )
...
// For now every .sid file is converted to byte array in /music subfolder , and added as #include"name_of_the_file.h" in SID.h
