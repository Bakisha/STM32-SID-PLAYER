...
inline void autoconfigMultiplier () {
  ...
  while (best_multiplier == 1) { //
    ...
    estimated_frame_time = ((20000 /* ms */  / multiplier) * (SID_uS + 2 /* irq overhead */ )) + (500 /* estimated irq instructions */ * (CPU_uS )); // estimated around 500 instruction per frame (20mS) should be enough for calculations
    if (estimated_frame_time < 13000) { // worst case: 13000 for bluepill at 48MHz, O0 (smallest code) optimatization
      best_multiplier = multiplier ;
    }
    // enable this if raw values are needed during calculations
    /*
  ...
  debugPrintTXT    ("SID emulator IRQ time:               ");
  debugPrintNUMBER (SID_uS);
  debugPrintTXTln  (" uS ");
