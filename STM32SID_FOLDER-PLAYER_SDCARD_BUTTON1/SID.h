...
uint32_t tmp_SID_data_size = 0;

uint16_t MagicID = 0;
uint16_t VERSION = 0;
uint16_t LOAD_ADDRESS = 0;
bool RAM_OVERFLOW = 0;
volatile bool play_me;

char SID_file_name[256] ; // reserve 256 bytes for file path name
char SID_DIR_name [256] ; // reserve 256 bytes for directory path name
char SID_filename [256] ; // reserve 256 bytes for filename only

char *sid_extension = "sid";
uint16_t file_count = 0;
volatile bool try_again = 0;
...
uint16_t SID_speed = 0;//20000; // (in uS) see how to calculate this number from sid header
...
volatile bool load_next_file = false;
uint32_t VIC_irq = 0;
...
uint8_t Gate_bit_1 = 0; //
uint8_t Gate_bit_2 = 0;
uint8_t Gate_bit_3 = 0;
/*
   Gate_bit_X variable:
